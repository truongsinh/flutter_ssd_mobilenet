...
class DetectionResultDisplay {
  ...
  static const zero = const DetectionResultDisplay(
    detections: [],
    imageSize: Size.zero,
...
Future<DetectionResultDisplay> detectObjectOnFrame({
  ...
  DetectionResultDisplay resultTuples = await response.first;
...
Future<tfl.Interpreter> loadModel({model, labels, numThreads}) async {
  // @todo Future.wait
...
echo(SendPort sendPort) async {
  ...
  await for (final msg in port) {
    if (msg[0] == "detect") {
      ...
      final CameraImage camImage = msg[2];
      final int numResultsPerClass = msg[3];
      final double threshold = msg[4];
      final tfl.Interpreter localInterpreter =
          tfl.Interpreter.fromSerialized(msg[5]);
